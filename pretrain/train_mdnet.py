...
def train_mdnet(opts):
    ...
    for i in range(opts['n_cycles']):
        ...
        states = {'shared_layers': model.layers.state_dict(), 'fc': model.fc.state_dict(), 'fc_a': model.fc_a.state_dict()}

        if i == 0:
            prec_old = prec.mean()
        else:
            if prec.mean() > prec_old and (i+1)%10 == 0:
                prec_old = prec.mean()
                torch.save(states, opts['model_path'].format(i+1))
